...
def load_context():
    ...
    if current_user.is_anonymous:
        g.search_form = None
    return dict(
        Action=action, search_form=g.search_form
    )
...
@login_required
def search():
    # how do I access the form without using g and is it too much work???
    try:
        if not g.search_form.validate():
            return redirect(url_for(Action.explore))
        page = request.args.get('page', 1, type=int)
        posts, total = Post.search(g.search_form.q.data, page,
                                   current_app.config['POSTS_PER_PAGE'])
        next_url = url_for(Action.search, q=g.search_form.q.data, page=page + 1) \
            if total > page * current_app.config['POSTS_PER_PAGE'] else None
        prev_url = url_for(Action.search, q=g.search_form.q.data, page=page - 1) \
            if page > 1 else None
        return render_template('search.html', title=_('Search'), posts=posts,
                               next_url=next_url, prev_url=prev_url)
    except RequestError:
        flash("The search feature is not working currently.")
